...
// JSHint generaría un error en este código debido a que el Símbolo no está definido en ES5.
// Definir este global en .jshintrc crearía un peligro de usar el global
// desprotegido en otro lugar, parece más seguro desactivar JSHint para estos
// tres líneas
/ * jshint ignorar: inicio * /
if (typeof Symbol === "function") {
	jQuery.fn [Symbol.iterator] = deletedIds [Symbol.iterator];
}
/ * jshint ignorar: final * /
...
function isArrayLike (obj) {
	...
	// Soporte: iOS 8.2 (no reproducible en el simulador)
		...
		"NIÑO": función (coincidencia) {
			...
			* /
			...
			if (match [1] .slice (0, 3) === "nth") {
				...
				? Partido [4] = + (partido [4] que coincida con [5] + (partido [6] || 1): 2 * (partido [3] === "incluso" || partido [3] === " impar" ) );
		...
		"CLASS": function (className) {
			...
			patrón de retorno ||
				...
				classCache (className, function (elem) {
					pattern.test volver (! typeof elem.className === "cadena" && || elem.className typeof elem.getAttribute == "indefinido" && elem.getAttribute ( "clase") || "");
				});
		},
...
función winnow (elementos, calificador, no) {
	...
	return jQuery.grep (elementos, función (elem) {
		return (jQuery.inArray (elem, calificador)> -1)! == no;
	});
}
				...
				if (lista) {
					...
					(función add (args) {
						jQuery.each (args, function (_, arg) {
							...
							} else if (arg && arg.length && jQuery.type (arg)! == "cadena") {
